...
document.getElementById('applicationForm').addEventListener('submit', async function (e) {
  ...
  try {
    ...
    const basicFields = [
      ...
      'kewarganegaraan', 'keturunan', 'npwp',
    ...
    document.getElementById('pendidikanContainer').querySelectorAll('.card-entry').forEach(entry => {
      ...
      const gelar = entry.querySelector(`[name="pendidikan_gelar_${idx}"]`)?.value || '';
      const ipk = entry.querySelector(`[name="pendidikan_ipk_${idx}"]`)?.value || '';
      const kualifikasi = entry.querySelector(`[name="pendidikan_kualifikasi_${idx}"]`)?.value || '';
      if (sekolah || dari || ke || jurusan || gelar || ipk || kualifikasi) {
        pendidikanEntries.push({ sekolah, dari, ke, jurusan, gelar, ipk, kualifikasi });
      }
    });
